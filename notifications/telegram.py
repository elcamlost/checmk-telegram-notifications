...
HOST_TEMPLATE = """<b>$NOTIFICATIONTYPE$: <a href="%s">$HOSTNAME$</a> $EVENT_TXT$</b>
<code>
Host:     $HOSTNAME$
Alias:    $HOSTALIAS$
Address:  $HOSTADDRESS$
...
SERVICE_TEMPLATE = """<b>$NOTIFICATIONTYPE$: <a href="%s">$HOSTNAME$/$SERVICEDESC$</a> $EVENT_TXT$</b>
<code>
Host:     $HOSTNAME$
Alias:    $HOSTALIAS$
Address:  $HOSTADDRESS$
...
def telegram_chat_id(context):
    ...
    raise AttributeError("Unable to find chat ID in any field: %s" %
                         ",".join(CHAT_ID_FIELD_NAMES))


def telegram_url_for(command, context, hide_token=False):
    return "https://api.telegram.org/bot%s/%s" % (
        telegram_bot_token(context) if not hide_token else "****", command)


def telegram_command(endpoint, context, files=None, **kwargs):
    json_data = dict({"chat_id": telegram_chat_id(context)}, **kwargs)

    if not files:
        response = requests.post(url=telegram_url_for(endpoint, context),
                                 json=json_data,
                                 files=files)
